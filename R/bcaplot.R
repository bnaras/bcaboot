...
#'     \code{bcajack2}, \code{bcanon}, or \code{bcapar} to plot bca
#'     and standard confidence limits for the parameter of interest.
#'
#' @param vl output of \code{bcajack}, \code{bcajack2}, \code{bcanon},
#'     or \code{bcapar}
...
bcaplot <- function(vl, main = " ", xlab = "coverage", ylab = "limits", alpha = c(0.025,
    ...
    ## Remove the alpha=0.5 (median) row if present — plot uses paired
    ## lower/upper limits only
    mid_row <- which(abs(as.numeric(rownames(vm)) - 0.5) < 1e-10)
    if (length(mid_row) == 1)
        vm <- vm[-mid_row, ]
    ...
    jn <- which(vn == "std")
    vja <- which(vn == "jacksd")
    has_jacksd <- length(vja) > 0 && !all(is.na(vm[, vja]))
    if (has_jacksd)
        jasd <- vm[, vja]

    ## Working matrix: bca (col 1) and std (col 2)
    vm <- vm[, c("bca", "std")]

    ## Get theta from stats matrix (first row, "theta" column)
    thet <- vl$stats[1, 1]
    ...
    if (add == 0) {
        ...
        for (k in seq_along(alp)) graphics::text(alp[k], thet, paste(100 * alp[k], "%", sep = ""),
            cex = 1.2)
        if (has_jacksd) {
