...
Much of the theory behind the approach can be found in references
@efron1987, @diciccio1992, @diciccio1996, and @efron2016. 
...
if (!requireNamespace("glmnet", quietly = TRUE)) {
    stop("Please install glmnet package")
}
...
them `resp` was of particular concern. So here $\theta$, the parameter
of interest is the coefficient of `resp`.
...
```{r}
knitr::kable(glm_bca$lims)
```

```{r}
knitr::kable(glm_bca$stats)
```
...
```{r}
as.matrix(coef(glmnet_model, s = glmnet_model$lambda.min)) %>%
    (function(x) data.frame(variable = rownames(x), coefficient = x[, 1])) %>%
...
```{r}
knitr::kable(glmnet_bca$lims)
```

```{r}
knitr::kable(glmnet_bca$stats)
```
...
```{r}
result$lims %>%
    cbind(exact = 1 / qf(df1 = n1, df2 = n2, p = 1 - as.numeric(rownames(result$lims)))) %>%
    knitr::kable()
```
...
```{r}
knitr::kable(result$abcstats)
```

```{r}
knitr::kable(result$ustats)
```
